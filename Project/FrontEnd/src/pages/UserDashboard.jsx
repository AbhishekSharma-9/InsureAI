...
import {
  ...
  UserIcon,
  LogoutIcon,
  CalendarIcon, // Added for scheduling
...
import MyPoliciesPage from './user/MyPoliciesPage';
import MyClaimsPage from './user/MyClaimsPage';
import ProfilePage from './user/ProfilePage';
import ScheduleAppointmentPage from './user/ScheduleAppointmentPage'; // <-- MILESTONE 2 PAGE

// Receives 'onLogout' from App.jsx
const UserDashboard = ({ onLogout }) => {
  // This state controls which sub-page is visible
  const [activeTab, setActiveTab] = useState('dashboard'); // 'dashboard', 'policies', 'claims', 'profile', 'schedule'
  ...
  const renderActiveTab = () => {
    switch (activeTab) {
      ...
      case 'profile':
        return <ProfilePage />;
      case 'schedule':
        return <ScheduleAppointmentPage />; // <-- MILESTONE 2
      case 'dashboard':
      default:
    ...
    <div className="flex min-h-screen bg-gray-100">
      ...
      <nav className="w-64 bg-white shadow-md flex flex-col flex-shrink-0">
        ...
        <ul className="mt-6 space-y-2 flex-grow">
          ...
          <NavLink tabName="schedule" icon={<CalendarIcon className="w-5 h-5" />}>
